'''
Module Name : datamesh_collibra_raw.py
...
def main():
    ...
    if (f'--{projectConstants.COLLIBRA_TOKEN}' in sys.argv):
        collibra_token = getResolvedOptions(sys.argv, [projectConstants.COLLIBRA_TOKEN])[projectConstants.COLLIBRA_TOKEN]

    if (f'--{projectConstants.JOB_MONITORING_URL}' in sys.argv):
        job_monitoring_url = getResolvedOptions(sys.argv, [projectConstants.JOB_MONITORING_URL])[projectConstants.JOB_MONITORING_URL]
    ...
    collibra_util.load_collibra_raw(job_env, aws_region, collibra_token, job_monitoring_url, collibra_url)
