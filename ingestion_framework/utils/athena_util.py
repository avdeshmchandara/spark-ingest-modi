...
class AthenaUtil:
    ...
    def add_partition_athena_raw_table(conf, data_path, env, app_name):
        ...
        print(f'Athena Table - add_partition_athena_raw_table - {pipeline_id} - Started')
        ...
        print(f'Athena Table - add_partition_athena_raw_table - {pipeline_id} - Executing the query - {qry} - Started')
        for client in client_list:
            ...
            if state == "FAILED":
                ...
                print(f'Athena Table - add_partition_athena_raw_table - {pipeline_id} - Executing the query - Failed')
                print(f'Athena Table - add_partition_athena_raw_table - {pipeline_id} - Error - {error_message}')
                raise Exception(f"Athena query execution failed with Error for table {pipeline_id} : {error_message}")
            else:
                print(f'Athena Table - add_partition_athena_raw_table - {pipeline_id} - Executing the query - Succeeded')
        print(f'Athena Table - add_partition_athena_raw_table - {pipeline_id} - Executing the query - Finished')


    @staticmethod
    def create_athena_raw_table(conf, data_path, env, app_name, is_partitioned=False):
        pipeline_id = conf[dynamodbConstants.PIPELINE_ID]
        print(f'Athena Table - create_athena_raw_table - {pipeline_id} - Started')
        print(f'Athena Table - create_athena_raw_table - {pipeline_id} - in regions {projectConstants.US_EAST_1} and {projectConstants.US_WEST_2}')
        athena_us_east_1_client = boto3.client(projectConstants.ATHENA, region_name=projectConstants.US_EAST_1)
        athena_us_west_2_client  = boto3.client(projectConstants.ATHENA, region_name=projectConstants.US_WEST_2)
        ...
        print(f'Athena Table - drop_athena_raw_table - {pipeline_id} - syntax - {drop_table_query}') 
        ...
        for client in client_list:
            ...
            print(f'Athena Table - create_athena_raw_table - {pipeline_id} -in aws region - {region_name} - Started')
            for query in athena_drop_create_list:
                print(f'Athena Table - create_athena_raw_table - {pipeline_id} - Executing the query - {query} - Started')
                res = client.start_query_execution(
                    QueryString=query,
                ...
                if state == "FAILED":
                    error_message = status['QueryExecution']['Status']['StateChangeReason']
                    print(f'Athena Table - create_athena_raw_table - {pipeline_id} - Executing the query - Failed')
                    print(f'Athena Table - create_athena_raw_table - {pipeline_id} - Error - {error_message}')
                    raise Exception(f"Athena query execution failed with Error for table {pipeline_id} : {error_message}")
                else:
                    print(f'Athena Table - create_athena_raw_table - {pipeline_id} - Executing the query - Succeeded')
                print(f'Athena Table - create_athena_raw_table - {pipeline_id} - Executing the query - Finished')
            print(f'Athena Table - create_athena_raw_table - {pipeline_id} -in aws region - {region_name} - Finished')
    ...
    def create_athena_processed_table(pipeline_id, df, data_path, app_name, domain_name, aws_region, env):
        ...
        for athena_client in athena_client_list:
            ...
            print(f'Athena Table - create_athena_processed_table - {pipeline_id} - in aws region - {region_name}- started')
            for query in athena_queries:
                print(f'Athena Table - create_athena_processed_table - {pipeline_id} - Executing the query - {query} - Started')
                ...
                if state == "FAILED":
                    error_message = status['QueryExecution']['Status']['StateChangeReason']
                    print(f'Athena Table - create_athena_processed_table - {pipeline_id} - Executing the query - Failed')
                    print(f'Athena Table - create_athena_processed_table - {pipeline_id} - Error - {error_message}')
                    raise Exception(f"Athena query execution failed with Error : {error_message}")
                else:
                    print(f'Athena Table - create_athena_processed_table - {pipeline_id} - Executing the query - Succeeded')
                print(f'Athena Table - create_athena_processed_table - {pipeline_id} - Executing the query - Finished')
            print(f'Athena Table - create_athena_processed_table - {pipeline_id} - in aws region - {region_name}- Finished')
